...
export default function AnnotationThread({ docPath }: Props) {
  ...
  const handleSaveEditDraft = (draftId: string) => {
    updateDraft(docPath, draftId, editDraftContent);
    setDrafts(prev => prev.map(d => d.id === draftId ? { ...d, content: editDraftContent } : d));
  ...
  const handleDeleteDraft = (draft: DraftComment) => {
    if (confirm('Delete this draft comment?')) {
      deleteDraft(docPath, draft.id);
    }
  };
                ...
                <>
                  ...
                  <div className="thread-replies">
                    ...
                    {authenticated && !isResolved && (
                      <button
                        className="thread-reply-btn-bottom"
                        onClick={(e) => { e.stopPropagation(); setReplyingTo(annotation.id); setReplyContent(''); }}
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/></svg>
                        Reply to thread...
                      ...
                      <div key={draft.id} className="thread-draft-item">
                        <div className="thread-draft-content">
                          ...
                          {editingDraftId === draft.id ? (
                            <div className="thread-draft-editor">
                              <textarea
                                className="thread-draft-editor__textarea"
                                value={editDraftContent}
                                onChange={(e) => setEditDraftContent(e.target.value)}
                                rows={3}
                                autoFocus
                              />
                              <div className="thread-draft-editor__actions">
                                <button
                                  className="thread-reply-editor__cancel"
                        ...
                        {editingDraftId !== draft.id && (
                          <div className="thread-draft-actions">
                            <button
                              className="thread-draft-action-btn"
                            ...
                            <button
                              className="thread-draft-action-btn thread-draft-action-btn--delete"
